...
static std::string sanitize_key(std::string_view key) {
    ...
    uint8_t hash[32];
    mbedtls_sha256(reinterpret_cast<const uint8_t*>(key.data()), key.size(), hash, 0);
...
void Esp32Storage::set(std::string_view key, std::span<const uint8_t> value) {
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Error setting blob '%s': %s", k.c_str(), esp_err_to_name(err));
    }
...
std::optional<std::vector<uint8_t>> Esp32Storage::get(std::string_view key) {
    ...
    if (err != ESP_OK && err != ESP_ERR_NVS_NOT_FOUND) {
        ESP_LOGE(TAG, "Error getting blob size '%s': %s", k.c_str(), esp_err_to_name(err));
        nvs_close(handle);
        return std::nullopt;
    }
