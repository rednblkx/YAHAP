...
void Esp32Crypto::sha512(std::span<const uint8_t> data, std::span<uint8_t, 64> output) {
    mbedtls_sha512(data.data(), data.size(), output.data(), 0);
}

void Esp32Crypto::hkdf_sha512(
...
void Esp32Crypto::ed25519_generate_keypair(std::span<uint8_t, 32> public_key, std::span<uint8_t, 64> private_key) {
    crypto_sign_ed25519_keypair(public_key.data(), private_key.data());
}

void Esp32Crypto::ed25519_sign(
    std::span<const uint8_t, 64> private_key,
    std::span<const uint8_t> message,
    std::span<uint8_t, 64> signature) 
{
    unsigned long long sig_len;
    crypto_sign_ed25519_detached(signature.data(), &sig_len, message.data(), message.size(), private_key.data());
}

bool Esp32Crypto::ed25519_verify(
    std::span<const uint8_t, 32> public_key,
    std::span<const uint8_t> message,
    std::span<const uint8_t, 64> signature) 
{
    return crypto_sign_ed25519_verify_detached(signature.data(), message.data(), message.size(), public_key.data()) == 0;
}

void Esp32Crypto::x25519_generate_keypair(std::span<uint8_t, 32> public_key, std::span<uint8_t, 32> private_key) {
...
{
    int ret = crypto_scalarmult(shared_secret.data(), private_key.data(), peer_public_key.data());
    (void)ret;
}
...
static void mpi_to_bytes_pad(const mbedtls_mpi* x, std::vector<uint8_t>& out, size_t pad_len) {
    out.resize(pad_len);
    size_t len = mbedtls_mpi_size(x);
    if (len > pad_len) len = pad_len;
    mbedtls_mpi_write_binary(x, out.data() + (pad_len - len), len);
}

static void hash_update(mbedtls_sha512_context* ctx, std::span<const uint8_t> data) {
...
std::unique_ptr<hap::platform::SRPSession> Esp32Crypto::srp_new_verifier(
    ...
    std::string_view password) 
{
    ...
    mbedtls_sha512_update(&ctx, (const uint8_t*)username.data(), username.size());
    mbedtls_sha512_update(&ctx, (const uint8_t*)":", 1);
    mbedtls_sha512_update(&ctx, (const uint8_t*)password.data(), password.size());
    ...
    return std::make_unique<Esp32SRPSession>(s, v_bytes, std::string(username), std::string(password));
}

std::array<uint8_t, 16> Esp32Crypto::srp_get_salt(hap::platform::SRPSession* session) {
    return session->salt;
}

std::vector<uint8_t> Esp32Crypto::srp_get_public_key(hap::platform::SRPSession* session) {
    ...
    mbedtls_mpi_init(&N); mbedtls_mpi_init(&g); mbedtls_mpi_init(&v);
    mbedtls_mpi_init(&b); mbedtls_mpi_init(&B);
    mbedtls_mpi_init(&k_mpi); mbedtls_mpi_init(&tmp);
    ...
    mbedtls_mpi_free(&N); mbedtls_mpi_free(&g); mbedtls_mpi_free(&v);
    mbedtls_mpi_free(&b); mbedtls_mpi_free(&B);
    mbedtls_mpi_free(&k_mpi); mbedtls_mpi_free(&tmp);

    session->server_public_key = ess->B;
    return ess->B;
}

bool Esp32Crypto::srp_set_client_public_key(hap::platform::SRPSession* session, std::span<const uint8_t> client_public) {
    session->client_public_key.assign(client_public.begin(), client_public.end());
    return true;
}

bool Esp32Crypto::srp_verify_client_proof(hap::platform::SRPSession* session, std::span<const uint8_t> proof) {
    auto ess = static_cast<Esp32SRPSession*>(session);
    
    mbedtls_mpi N, A, B, v, b, S, u_mpi;
    mbedtls_mpi_init(&N); mbedtls_mpi_init(&A); mbedtls_mpi_init(&B);
    mbedtls_mpi_init(&v); mbedtls_mpi_init(&b); mbedtls_mpi_init(&S);
    mbedtls_mpi_init(&u_mpi);
    
    mbedtls_mpi_read_string(&N, 16, SRP_N_HEX);
    mbedtls_mpi_read_binary(&A, session->client_public_key.data(), session->client_public_key.size());
    mbedtls_mpi_read_binary(&B, ess->B.data(), ess->B.size());
    mbedtls_mpi_read_binary(&v, ess->verifier.data(), ess->verifier.size());
    mbedtls_mpi_read_binary(&b, ess->b.data(), ess->b.size());
    ...
    mbedtls_mpi g_mpi; mbedtls_mpi_init(&g_mpi); mbedtls_mpi_read_string(&g_mpi, 16, SRP_g_HEX);
    mpi_to_bytes(&N, N_bytes);
    mpi_to_bytes(&g_mpi, g_bytes);
    
    mbedtls_sha512_starts(&ctx, 0); hash_update(&ctx, N_bytes); mbedtls_sha512_finish(&ctx, hN.data());
    mbedtls_sha512_starts(&ctx, 0); hash_update(&ctx, g_bytes); mbedtls_sha512_finish(&ctx, hg.data());
    mbedtls_sha512_starts(&ctx, 0); mbedtls_sha512_update(&ctx, (const uint8_t*)session->username.data(), session->username.size()); mbedtls_sha512_finish(&ctx, hI.data());
    ...
    if (proof.size() != M1_calc.size() || std::memcmp(proof.data(), M1_calc.data(), proof.size()) != 0) {
        ESP_LOGE(TAG, "SRP Client Proof Failed");
        mbedtls_mpi_free(&N); mbedtls_mpi_free(&A); mbedtls_mpi_free(&B);
        mbedtls_mpi_free(&v); mbedtls_mpi_free(&b); mbedtls_mpi_free(&S); mbedtls_mpi_free(&u_mpi); mbedtls_mpi_free(&g_mpi);
        mbedtls_sha512_free(&ctx);
        return false;
    }
    ...
    ESP_LOGI(TAG, "Client Public Key (Received): %d bytes", (int)session->client_public_key.size());
    ESP_LOG_BUFFER_HEX(TAG, session->client_public_key.data(), std::min((size_t)64, session->client_public_key.size()));
    ESP_LOGI(TAG, "A_padded: %d bytes", (int)A_padded.size());
    ESP_LOG_BUFFER_HEX(TAG, A_padded.data(), std::min((size_t)64, A_padded.size()));
...
std::vector<uint8_t> Esp32Crypto::srp_get_server_proof(hap::platform::SRPSession* session) {
    return session->M2;
}

std::vector<uint8_t> Esp32Crypto::srp_get_session_key(hap::platform::SRPSession* session) {
    auto ess = static_cast<Esp32SRPSession*>(session);
    return ess->K;
}
